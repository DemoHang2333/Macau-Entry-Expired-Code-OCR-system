...
class Splitter(object):
    ...
    # 计算投影分割点
    def extract_peek_ranges_from_array(self, array_vals, minimun_val=5, minimun_range=2,judge="normal",judge_mini_value=0):
        ...
        for i, val in enumerate(array_vals):
            if val > minimun_val and start_i is None:
                ...
                end_i = i
                if index==0 and judge=="vertical":
                    width=end_i-start_i
                    if width<30:
                        width=0
                    else:
                        index=1
                if width!=0 and float(end_i - start_i)>1.5*width:
                    peek_ranges.append((start_i, int((end_i-start_i)/2)+start_i))
                    peek_ranges.append((int((end_i-start_i)/2)+start_i+1,end_i))
                    start_i = None
                elif end_i - start_i >= minimun_range:
    ...
    def process_by_img(self, image_color, result_img_path, minimun_range=11, sub_segment=False, pred_val_list=[]):
        ...
        while i<len(vertical_peek_ranges):
            if vertical_peek_ranges[i][1]-vertical_peek_ranges[i][0]>50 and row_infor[i][1]-row_infor[i][0]>25:
                ...
                if vertical_peek_ranges[i][1] - vertical_peek_ranges[i][0] > 170 and not (0 in horizontal_sum):
                    ...
                    peek_ranges = self.extract_peek_ranges_from_array(horizontal_sum, judge="row",minimun_val=minimun_val+1)
                else:
                    peek_ranges = self.extract_peek_ranges_from_array(horizontal_sum, judge="row")
                ...
                new_vertical_peek_ranges = self.extract_peek_ranges_from_array(vertical_sum,judge="vertical")
        ...
        while j < len(vertical_peek_ranges):
            ...
            pt1 = (x, y)
            pt2 = (x + w, y + h)
            if h<15 or w<15 or h>300 or w>300 :
